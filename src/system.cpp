...
bool System::LoadObjects(ObjectManager& objm) //메인 루프 전에 한번만 호출함.
{
    ...
    objm.mSubMap = new Map(System::sWindowWidth*0.5 - 6*40, 100, 16, 16, 80); //서브맵 객체 생성
...
bool System::LoadUIs(UIManager& uim, ObjectManager& objm)
{
    bool success = true;
...
Uint64 Timer::StoreProgramTickMs()
{
    mMs = SDL_GetTicks();
